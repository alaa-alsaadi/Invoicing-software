                ...
                <div class="card-header d-flex">
                    ...
                    <div class="table-responsive">
                        <table class="table card-table">
                            ...
                            <tr>
                                <td><a href="{{ route('invoice.show', $invoice->id) }}" style="color:inherit">{{ $invoice->customer_name }}</a></td>
                                <td>{{ $invoice->invoice_date }}</td>
                                <td>{{ $invoice->total_due }}</td>
                                <td>
                                    <a href="{{ route('invoice.edit', $invoice->id) }}" class="btn btn-primary btn-sm">
                                        <i class="fa fa-edit"></i>{{ __('Frontend/frontend.edit') }}
                                    </a>
                                    <a href="javascript:void(0)" onclick="if (confirm('{{ __('Frontend/frontend.r_u_sure') }}')) { document.getElementById('delete-{{ $invoice->id }}').submit(); } else { return false; }" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i>{{ __('Frontend/frontend.delete') }}</a>
